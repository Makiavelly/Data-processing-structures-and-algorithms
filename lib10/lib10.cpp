...
list* add_elem(list* head, tree* tree, int data) {
...
void symmetric_view_not_recursion(tree* root) {
	bool stop{ false };
	tree* temp_tree{ root };
	int depth{ 0 };
	...
	while (!stop) {
		while (temp_tree != nullptr) {
			sp = add_elem(sp, temp_tree, depth);
			temp_tree = temp_tree->left;
			depth += 1;
		}
		if (sp == nullptr) stop = true;
		else {
			for (int i = 0; i < sp->depth; i++) std::cout << "     ";
			std::cout << sp->tree->data << std::endl;
			temp_tree = sp->tree->right;
			depth = sp->depth + 1;
			sp = delete_elem(sp);
		}
	}
...
int main()
{
	...
	std::cout << std::endl;
	setConsoleColor(1);
	std::cout << "Симметричный обход (нерекурсивный)" << std::endl;
	symmetric_view_not_recursion(root);
	std::cout << std::endl;
	setConsoleColor(7);
}